...
class TenderModel extends Model{
	...
	public function getList($condition = array(), $offset = 0, $perpage = 10, $orderby = '') {
		...
		$orderby = empty($orderby) ? 'create_time desc' : $orderby;
	...
	public function addData($data){	
		...
		if((int)$res > 0){
			//推送投标信息(通知有人投标)
			D('PushDemandinfo')->pushInfo(1,$demandInfo['id'],$demandInfo['uid']);
	...
	public function updateStatus($data){
		...
		if($res !== false){
			...
			//将求购信息中的status写成1
			$demand->updateField(array('id' => $data['demand_id'],'status' => 1));
			//推送投标信息(通知投标人中标)
			D('PushDemandinfo')->pushInfo(2,$demandInfo['id'],$already['uid']);
