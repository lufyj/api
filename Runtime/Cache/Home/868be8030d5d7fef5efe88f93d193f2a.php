...
<html>
	...
	<div class="detail-middle">
		...
		<div class="drugs-main clearfix">
			<?php if(is_array($list)): foreach($list as $key=>$vo): ?><div class="competes clearfix l">
					<div class="competes-img l">
						<a href="<?php echo U('demand/detail',array('id'=>$vo['id']));?>">
							<img src="<?php echo ((isset($vo["goods_img"]) && ($vo["goods_img"] !== ""))?($vo["goods_img"]):'/Public/Home/images/noimg.png'); ?>" />
						</a>
					</div>
					<div class="competes-detail l">
						<ul class="clearfix">
							<li>商品名称：<?php echo ($vo["goods_name"]); ?></li>
							<li>商品规格：<?php echo ($vo["goods_attr_name"]); ?></li>
							<li>求购数量：<?php echo ($vo["num"]); ?>公斤</li>
							<li class="cd-color">联&nbsp;&nbsp;系&nbsp;&nbsp;人：<?php echo ($vo["contacts"]); ?></li>
							<li class="cd-color">联系电话：<?php echo ($vo["mobile"]); ?></li>
							<li class="cd-color">发布时间：<?php echo (date('Y-m-d',$vo["create_time"])); ?></li>
						</ul>
					</div>
	...
	<script type="text/javascript">
		$(function() {
			...
			/* 加载求购列表页 */
		function rload() {
			window.location.href = "<?php echo U('demand/lst');?>";
		}
