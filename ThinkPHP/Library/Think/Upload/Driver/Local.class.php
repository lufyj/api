...
class Local{
    ...
    public function save($file, $replace=true) {
        ...
        $resizeImg = new \Org\Com\ImageResize;
        $resizeImg->load($file['tmp_name']);
        if($file['only']){
        	$resizeImg->size($file['only'][0], $file['only'][1]);        		
        }
        $resizeImg->fixed_given_size(false)
        	->keep_ratio(true)
        	->bg_color("#ffffff")
        	->quality(50)
        	->save($filename);
       ...
       /*  $img->load($file['tmp_name'])
        	...
        	->rotate(20)    //指定旋转的角度
        	->quality(46)   //设置生成图片的质量 0-100，如果生成的图片格式为png格式，数字越大，压缩越大，如果是其他格式，如jpg，gif，数组越小，压缩越大
        	->set_watermark($watermark)     //添加水印
        	->save($filename);   //保存生成图片的路径 */
